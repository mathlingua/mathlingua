...
fun neoValidateMatchingSection(
    node: Phase1Node, errors: MutableList<ParseError>, tracker: MutableLocationTracker
) =
    neoTrack(node, tracker) {
        neoValidateSection(node.resolve(), errors, "matching", DEFAULT_MATCHING_SECTION) {
            MatchingSection(clauses = neoValidateClauseListNode(it, errors, tracker))
        }
    }
