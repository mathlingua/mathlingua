...
fun neoValidateDefinesCollectsGroup(
    node: Phase1Node, errors: MutableList<ParseError>, tracker: MutableLocationTracker
) =
    neoTrack(node, tracker) {
        neoValidateGroup(node.resolve(), errors, "Defines", DEFAULT_DEFINES_COLLECTS_GROUP) {
        group ->
            neoIdentifySections(
                group,
                errors,
                ...
                DefinesCollectsGroup(
                    ...
                    collectsSection =
                        neoEnsureNonNull(sections["collects"], DEFAULT_COLLECTS_SECTION) {
                            neoValidateCollectsSection(it, errors, tracker)
                        },
                    usingSection =
