...
data class DefinitionSection(val clauses: ClauseListNode) : Phase2Node {
    override fun forEach(fn: (node: Phase2Node) -> Unit) = clauses.forEach(fn)

    override fun toCode(isArg: Boolean, indent: Int, writer: CodeWriter): CodeWriter {
        writer.writeIndent(isArg, indent)
        writer.writeHeader("Definition")
    ...
    override fun transform(chalkTransformer: (node: Phase2Node) -> Phase2Node) =
        chalkTransformer(DefinitionSection(
            clauses = clauses.transform(chalkTransformer) as ClauseListNode
        ))
}

fun validateDefinitionSection(node: Phase1Node, tracker: MutableLocationTracker) = validateClauseList(
    tracker,
    node,
    "Definition",
    false,
