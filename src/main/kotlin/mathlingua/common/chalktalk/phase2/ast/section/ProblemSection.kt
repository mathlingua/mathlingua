...
data class ProblemSection(val clauses: ClauseListNode) : Phase2Node {
    override fun forEach(fn: (node: Phase2Node) -> Unit) = clauses.forEach(fn)

    override fun toCode(isArg: Boolean, indent: Int, writer: CodeWriter): CodeWriter {
        writer.writeIndent(isArg, indent)
        writer.writeHeader("Problem")
...
fun validateProblemSection(node: Phase1Node, tracker: MutableLocationTracker) = validateClauseList(
    tracker,
    node,
    "Problem",
    false,
