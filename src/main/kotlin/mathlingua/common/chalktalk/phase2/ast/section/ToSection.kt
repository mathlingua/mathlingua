...
data class ToSection(val statement: Statement) : Phase2Node {
    ...
    override fun transform(chalkTransformer: (node: Phase2Node) -> Phase2Node) =
        chalkTransformer(ToSection(
            statement = statement.transform(chalkTransformer) as Statement
        ))
}

fun validateToSection(node: Phase1Node, tracker: MutableLocationTracker) = validateStatementSection(
    node,
    tracker,
    "to",
    ::ToSection
