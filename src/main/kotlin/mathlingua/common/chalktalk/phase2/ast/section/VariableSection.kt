...
data class VariableSection(val variables: List<String>) : Phase2Node {
    override fun forEach(fn: (node: Phase2Node) -> Unit) {
    }

    override fun toCode(isArg: Boolean, indent: Int, writer: CodeWriter): CodeWriter {
        writer.writeIndent(isArg, indent)
        writer.writeHeader("variable")
        writer.writeNewline()
...
fun validateVariableSection(
    rawNode: Phase1Node,
    tracker: MutableLocationTracker
): Validation<VariableSection> {
    ...
    if (sect.name.text != "variable") {
        errors.add(
            ParseError(
                "Expected a Section with name 'variable' but found " + sect.name.text,
                row, column
            )
        )
    }
    ...
    for (arg in sect.args) {
        ...
        variables.add((arg.chalkTalkTarget as Phase1Token).text)
    }
