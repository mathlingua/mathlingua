...
internal data class HaveSection(
...
internal fun validateHaveSection(node: Phase1Node, errors: MutableList<ParseError>) =
    validateSection(node.resolve(), errors, "have", DEFAULT_HAVE_SECTION) {
        HaveSection(clauses = validateClauseListNode(it, errors), node.row, node.column)
    }
