...
internal data class ThenSection(
...
internal fun isThenSection(sec: Section) = sec.name.text == "then"

internal fun validateThenSection(node: Phase1Node, errors: MutableList<ParseError>) =
    validateSection(node.resolve(), errors, "then", DEFAULT_THEN_SECTION) {
        ThenSection(clauses = validateClauseListNode(it, errors), node.row, node.column)
    }
