...
internal data class WithSection(val clauses: ClauseListNode) : Phase2Node {
    ...
    override fun transform(chalkTransformer: (node: Phase2Node) -> Phase2Node) =
        chalkTransformer(
            WithSection(clauses = clauses.transform(chalkTransformer) as ClauseListNode))
}

internal fun isWithSection(sec: Section) = sec.name.text == "with"

internal fun validateWithSection(
    node: Phase1Node, errors: MutableList<ParseError>, tracker: MutableLocationTracker
) =
    track(node, tracker) {
        validateSection(node.resolve(), errors, "with", DEFAULT_WITH_SECTION) {
            WithSection(clauses = validateClauseListNode(it, errors, tracker))
        }
    }
