...
internal data class OrSection(
    ...
    override fun transform(chalkTransformer: (node: Phase2Node) -> Phase2Node) =
        chalkTransformer(
            OrSection(clauses = clauses.transform(chalkTransformer) as ClauseListNode, row, column))
}

internal fun validateOrSection(node: Phase1Node, errors: MutableList<ParseError>) =
    validateSection(node.resolve(), errors, "or", DEFAULT_OR_SECTION) {
        OrSection(clauses = validateClauseListNode(it, errors), node.row, node.column)
    }
