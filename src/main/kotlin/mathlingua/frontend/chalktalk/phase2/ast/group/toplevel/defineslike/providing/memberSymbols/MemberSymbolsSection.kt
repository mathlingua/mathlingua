...
internal data class MemberSymbolsSection(
    ...
    override fun toCode(isArg: Boolean, indent: Int, writer: CodeWriter): CodeWriter {
        ...
        writer.writeHeader("memberSymbols")
...
internal fun validateMemberSymbolsSection(node: Phase1Node, errors: MutableList<ParseError>) =
    validateTargetSection(
        node.resolve(),
        errors,
        "memberSymbols",
        DEFAULT_MEMBER_SYMBOLS_SECTION,
        node.row,
        node.column) { targets, row, column ->
        MemberSymbolsSection(targets = targets, row, column)
    }
