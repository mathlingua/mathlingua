...
internal data class ThatSection(
...
internal fun validateThatSection(node: Phase1Node, errors: MutableList<ParseError>) =
    validateSection(node.resolve(), errors, "that", DEFAULT_THAT_SECTION) {
        ThatSection(clauses = validateClauseListNode(it, errors), node.row, node.column)
    }
