...
data class OfSection(val statement: Statement) : Phase2Node {
    ...
    override fun transform(chalkTransformer: (node: Phase2Node) -> Phase2Node) =
        chalkTransformer(OfSection(statement = chalkTransformer(statement) as Statement))
}
...
fun validateOfSection(
    ...
    track(node, tracker) {
        validateSection(node.resolve(), errors, "of", DEFAULT_OF_SECTION) {
            validateSingleArg(it, errors, DEFAULT_OF_SECTION, "statement") {
