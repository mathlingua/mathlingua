...
import mathlingua.frontend.chalktalk.phase2.ast.DEFAULT_CONTEXT_SECTION
...
data class ContextSection(val clauses: ClauseListNode) : Phase2Node {
...
fun isContextSection(sec: Section) = sec.name.text == "context"

fun validateContextSection(
    node: Phase1Node, errors: MutableList<ParseError>, tracker: MutableLocationTracker
) =
    track(node, tracker) {
        validateSection(node.resolve(), errors, "context", DEFAULT_CONTEXT_SECTION) {
            ContextSection(clauses = validateClauseListNode(it, errors, tracker))
        }
    }
