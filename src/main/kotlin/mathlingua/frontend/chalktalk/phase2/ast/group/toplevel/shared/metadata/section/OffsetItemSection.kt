...
import mathlingua.frontend.chalktalk.phase2.ast.DEFAULT_OFFSET_ITEM_SECTION
...
internal data class OffsetItemSection(
    val offset: String, override val row: Int, override val column: Int
) : Phase2Node {
...
internal fun validateOffsetItemSection(node: Phase1Node, errors: MutableList<ParseError>) =
    validateSection(node, errors, "offset", DEFAULT_OFFSET_ITEM_SECTION) { section ->
        validateSingleArg(section, errors, DEFAULT_OFFSET_ITEM_SECTION, "string") { arg ->
            ...
            if (resolved !is Phase1Token || resolved.type != ChalkTalkTokenType.String) {
                ...
                OffsetItemSection(offset = resolved.text, node.row, node.column)
            }
        }
    }
