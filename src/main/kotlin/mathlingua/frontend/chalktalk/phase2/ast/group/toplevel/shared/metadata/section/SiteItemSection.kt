...
internal data class SiteItemSection(
    val url: String, override val row: Int, override val column: Int
) : Phase2Node {
...
internal fun validateSiteItemSection(node: Phase1Node, errors: MutableList<ParseError>) =
    validateSection(node, errors, "site", DEFAULT_SITE_ITEM_SECTION) { section ->
        validateSingleArg(section, errors, DEFAULT_SITE_ITEM_SECTION, "string") { arg ->
            ...
            if (resolved !is Phase1Token || resolved.type != ChalkTalkTokenType.String) {
                ...
                SiteItemSection(url = resolved.text, node.row, node.column)
            }
        }
    }
