...
internal data class SourceItemSection(
    val sourceReference: String, override val row: Int, override val column: Int
) : Phase2Node {
...
internal fun validateSourceItemSection(node: Phase1Node, errors: MutableList<ParseError>) =
    validateSection(node, errors, "source", DEFAULT_SOURCE_ITEM_SECTION) { section ->
        validateSingleArg(section, errors, DEFAULT_SOURCE_ITEM_SECTION, "string") { arg ->
            ...
            if (resolved !is Phase1Token || resolved.type != ChalkTalkTokenType.String) {
                ...
                SourceItemSection(sourceReference = resolved.text, node.row, node.column)
            }
        }
    }
